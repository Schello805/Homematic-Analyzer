...
export function SignalQualityDeviceList({ devices, source, onSourceChange, receiverOptions }: {
  ...
  const visibleDevices = (showAll ? sourceDevices : attentionDevices).sort((left, right) => {
  ...
  const selectedDevice = visibleDevices.find((device) => device.key === selectedKey);
  ...
  return (
    <>
      <div className="signal-source-switch" role="group" aria-label="Signalquelle auswählen">
        <button type="button" className={source === "ccu" ? "is-active" : ""} onClick={() => onSourceChange("ccu")}>Ohne Snifferwerte <small>{devices.filter((device) => device.ccuRssi !== undefined).length}</small></button>
        <button type="button" className={source === "both" ? "is-active" : ""} onClick={() => onSourceChange("both")}>Mit Snifferwerten <small>{devices.filter((device) => device.ccuRssi !== undefined || device.snifferRssi !== undefined).length}</small></button>
      </div>
      ...
      {selectedDevice && (
        <section className="signal-improvement-plan">
